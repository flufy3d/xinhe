...
约定:
    data/<version>/<stage>/<split>.jsonl
    → HF config_name = "<version>_<stage>", split = <split>（val 自动映射 validation）
...
ds = load_dataset("flufy3d/xinhe-dataset", "v8_stage0", split="train")
print(ds[0]["conversations"])
```

config_name 命名：`<version>_<stage>`（如 `v8_stage0`、`v8_stage1`）。每个 config 含 `train` / `validation`，部分还含 `train_codex`（codex-flavored 变体）。
...
def discover_configs(data_root: Path):
    """扫 data/<version>/<stage>/<split>.jsonl, 聚合成 {(version, stage): [(split, rel_path), ...]}。"""
    ...
    for jsonl in sorted(data_root.rglob("*.jsonl")):
        ...
        if len(parts) != 3:
            ...
            continue
        version, stage, fname = parts
        split_raw = Path(fname).stem
        split = SPLIT_NAME_REMAP.get(split_raw, split_raw)
        rel = jsonl.relative_to(PROJECT_ROOT).as_posix()
        configs[(version, stage)].append((split, rel))
    if skipped:
        print(f"⚠ 跳过 {len(skipped)} 个不符合 data/<version>/<stage>/<split>.jsonl 模式的文件:")
        for p in skipped:
            print(f"    {p}")
    return configs
...
def build_yaml(configs) -> str:
    ...
    for (version, stage), splits in sorted(configs.items()):
        lines.append(f"  - config_name: {version}_{stage}")

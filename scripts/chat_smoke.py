"""
人工验收脚本 — 测 v7 Hippocampus 模型是否修好核心问题。

用法:
    python scripts/chat_smoke.py --ckpt checkpoints/curriculum/1_persona_unified.pt

测试场景:
    [A] 世界知识: 单轮 QA，无状态
    [B] 拒答: 空状态 + 问未告知信息 → 应 refuse
    [C] 多 fact 单句: 一句话 3 fact → 依次召回
    [D] 跨轮 overwrite: 事实覆写 → 召回最新值
    [E] 单槽穿插召回: 事实 + 几轮 chat + recall
    [F] 多槽 retention: 先后告知 2-3 槽 + chat + 依次 recall
"""
import argparse
import re
import sys
...
def test_G_verbatim(model, tokenizer, device):
    """Verbatim 复述：记一段随机 phrase → 插 distractor → 原样复述。
    测试 Hippocampus 对"非 persona 槽"连续 token 序列的 retention。"""
    print("=" * 60)
    print("[G] Verbatim 整段复述（随机 phrase + distractor + 原样召回）")
    print("=" * 60)
    cases = [
    ...
    for setup, recall_q, expected in cases:
        ...
        hit += int(ok)
    total = len(cases)
    print(f"[G] Verbatim 复述: {hit}/{total}")
    return hit, total


def test_H_meta_recall(model, tokenizer, device):
    """元认知自指：用户刚说一句自然陈述 → 插 distractor → 问"我刚才说了什么"。
    测试模型的"回看上一轮"能力（非 persona 槽）。"""
    print("=" * 60)
    print("[H] 元认知自指召回（我刚才说了什么）")
    print("=" * 60)
    cases = [
...
def main():
    ...
    p.add_argument("--ckpt", type=str,
                   default="checkpoints/curriculum/1_persona_unified.pt")
    ...
    print(f"  [G] Verbatim 复述:    {g_hit}/{g_total}   ← 非 persona 槽的整段记忆")
    print(f"  [H] 元认知自指:       {h_hit}/{h_total}   ← '我刚才说了什么'")
    total_hit = a_hit + b_hit + c_hit + d_hit + e_hit + f_hit + g_hit + h_hit
    total = a_total + b_total + c_total + d_total + e_total + f_total + g_total + h_total

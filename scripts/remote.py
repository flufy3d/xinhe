...
def cmd_deploy(args):
    ...
    # 3. 远端初始化（幂等）
    print("\n[3/4] 远端依赖...")
    # UV_HTTP_TIMEOUT=600: download.pytorch.org 上 triton/torch 大 wheel 30s 默认会 timeout
    # UV_DEFAULT_INDEX 清华: pypi.org 国内直连不稳;torch 仍走 pyproject explicit index
    init_script = f"""set -e
cd {remote_dir}
ENV_LINES='
...
echo "$ENV_LINES" | while read -r line; do
  [ -z "$line" ] && continue
...
# --inexact: 保留 uv pip 装的 causal-conv1d,否则每次 deploy 重编 30+ 分钟
echo '  uv sync...'
uv sync --inexact

if ! uv pip show causal-conv1d >/dev/null 2>&1; then
  # 限当前卡 PTX,默认会编 sm_75/80/.../121 全架构耗时 5-7x
  ARCH=$(nvidia-smi --query-gpu=compute_cap --format=csv,noheader 2>/dev/null | head -1 | tr -d ' ')
  [ -n "$ARCH" ] && export TORCH_CUDA_ARCH_LIST="$ARCH"
  echo "  编译 causal-conv1d${{ARCH:+ (sm_${{ARCH//./}})}}..."

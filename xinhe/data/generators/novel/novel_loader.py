...
# 默认章节标题 regex(union 多种常见格式,覆盖网络小说 + 现代文学常见排版)。
# 用户可在 yaml 里配 `chapter_pattern: "..."` 整体覆盖。
# 整体策略:不依赖具体书名,仅靠格式范式自动识别。
DEFAULT_CHAPTER_PATTERN = (
    r"^("
    # 中式章节标记:第X章/回/节/集/卷/部/篇(带"第")
    r"第[一二三四五六七八九十百千零〇\d]+[章回节集卷部篇]"
    # 不带"第"的部/篇/卷:"上 部 面壁者" / "中 篇 ..." / "下 卷"(空格强制,避正文"下部分"误伤)
    r"|[上中下前后][\s　]+[部篇卷]([\s　]+\S|$)"
    # 中文数字 + 部/篇/卷(无"第"):"一 部" / "二 篇" — 历史文学排版
    r"|[一二三四五六七八九十][\s　]+[部篇卷]([\s　]+\S|$)"
    # 阿拉伯数字编号:1. 标题 / 1．标题(全角点)/ 1。标题
    r"|\d+[.．。]\s*\S"
    # 英文章节
    r"|[Cc]hapter\s+\d+\b"
    # 短章节标记(行首 + 短词,行尾对齐避免误伤正文)
    r"|序章$|楔子$|引子$|尾声$|尾章$|序言$|序$|后记$|附录$|番外$|外传$"
    # 中括号短标题型(常见:【时间·事件】/【XXX】,限 2-40 字内 + 行尾,避免对话引述)
    r"|【[^】]{2,40}】$"
    r")"
)

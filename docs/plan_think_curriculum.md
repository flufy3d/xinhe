...
**比例**：占 think 训练数据的 **10%**。太多会稀释 state 读取信号，太少没效果。
...
- **50% 记忆 episode**（现有课程数据）→ 保持 state 读写能力
- **50% think episode**（本方案数据）→ 恢复推理 + 长回复 + 续聊能力

Think episode 内部比例：

| 类型 | 比例（占 think） | 说明 |
|------|---------|------|
| fact 推理（单问/总结/比较/组合） | 55% | 核心能力，从 state 读 fact 推理回答 |
| 上下文续聊 | 20% | 从 state 读对话上下文，自然接话 |
| 心跳包 | 15% | 空输入，基于 state 自主表达 |
| 纯逻辑 | 10% | LoRA 透明度，不依赖 state |

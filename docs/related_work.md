...
| | RxT | 心核 |
|---|---|---|
| 记忆读写 | cross-attention + 独立 memory encoder | cross-attention via past_key_values（零侵入 backbone） |
...
**评价**：v2 心核和 RxT 都用 cross-attention，但实现方式不同。心核通过 past_key_values API 注入标准 attention，不需要额外的 memory encoder，更轻量且与 Flash Attention 兼容。gate 从 GRU 风格简化为纯动态决策，长期记忆交给 sleep/MLP。
...
| 记忆形式 | per-layer KV 压缩（线性 attention） | per-layer KV 注入（专用投影 via past_key_values） |
...
| | EM-LLM | 心核 |
|---|---|---|
| **存储方式** | KV block 卸载到 CPU/磁盘，按需检索回 GPU | state 始终在 GPU 上，通过 KV-Cache 注入参与每次 forward |
...
| 心核组件 | 最近的先例 | 心核的扩展 |
|----------|-----------|-----------|
| 对称 cross-attention state | RMT (2022), RxT (2025) | 读写对称（Q/K/V 角色互换）+ past_key_values 零侵入 + 每层专用投影 |

...
- **心核** = 内存 + 演化 + 睡眠固化：state 通过对称 cross-attention 持续压缩记忆，sleep 时固化到 LoRA 权重，.pt = 灵魂
...
| | RMT | 心核 |
|---|---|---|
| 记忆读写 | [mem] token 混入序列，self-attention | 对称 cross-attention，state 不在序列中 |
| K/V 投影 | 共享 backbone 权重 | 每层独立专用投影（全参数，不受 LoRA 限制） |
| 更新机制 | 直接覆写 | 动态 gate：σ(gate_proj([old, new])) |
| LoRA | 无，全量微调或从零训 | LoRA（语言适配）+ Memory MLP（长期记忆） |
| Sleep | 无 | replay buffer + state 弱化 → 权重固化 |
| Per-user 分化 | 无 | .pt 灵魂分化 |

**评价**：RMT 是心核的架构起点。心核 v1 沿用了 state-as-tokens 思路并在 0.8B 上验证成功，但 4B 上暴露 LoRA 瓶颈。v2 改用对称 cross-attention + 专用投影，彻底解耦 state 路由与语言适配。
...
| | GDWM | 心核 |
|---|---|---|
| 权重固化 | LoRA adapter，按信息效用门控写入 | 专用 Memory MLP，sleep 时 replay + state 弱化 |
| 适用范围 | 单次长上下文内 | 跨 session，持续积累 |
| State token | 无 | 有，32×1024 |
| Sleep 周期 | 无，持续适应 | 有，批量巩固 |

**评价**：GDWM 是最接近心核 sleep 机制的已发表工作——都是将信息固化到可学习权重中。区别是 GDWM 用 LoRA 挂在 backbone 上，心核用独立的 Memory MLP；GDWM 没有跨 session 持久记忆。
...
| | RxT | 心核 |
|---|---|---|
| 记忆读写 | cross-attention + 独立 memory encoder | cross-attention via layer_hook（零侵入 backbone） |
| K/V 投影 | memory encoder 生成 | 每层独立专用投影 |
| Gate | GRU 风格 | σ(gate_proj([old, new]))，纯动态 |
| 额外网络 | 单独的 memory encoder | 专用 read/write 投影（轻量，复用 backbone attention） |

**评价**：v2 心核和 RxT 都用 cross-attention，但实现方式不同。心核通过 layer_hook 在每层之前执行显式 cross-attention，不需要额外的 memory encoder，更轻量且对混合架构（DeltaNet + full attention）统一生效。gate 从 GRU 风格简化为纯动态决策，长期记忆交给 sleep/MLP。
...
| 记忆形式 | per-layer KV 压缩（线性 attention） | per-layer KV 注入（专用投影 via layer_hook cross-attention） |
| 记忆更新 | 线性 attention 持续压缩 | gate 决策 + sleep 固化 |
| 在线学习 | 无 | Sleep 权重更新 |

**评价**：v2 心核的 per-layer KV 注入与 Infini-attention 的 per-layer 压缩记忆表面相似，但本质不同。Infini-attention 用线性 attention 做 KV 压缩（单次上下文内），心核用专用投影从显式 state 生成 KV（跨会话持久）。心核的 state 是可持久化的外部记忆，不是上下文内的压缩缓存。
...
| **记忆形态** | 显式存储的 KV 对（episodic memory blocks） | 32 个 state slot 通过对称 cross-attention 持续演化 |

### 训练 vs 推理

| | EM-LLM | 心核 |
|---|---|---|
| **是否需要训练** | 完全免训练（inference-time monkey-patch） | 需要训练（LoRA + StateInterface，~5M 参数） |
...
| | EM-LLM | 心核 |
|---|---|---|
| **存储方式** | KV block 卸载到 CPU/磁盘，按需检索回 GPU | state 始终在 GPU 上，通过 layer_hook 注入参与每次 forward |
...
| **遗忘机制** | 无主动遗忘，所有 block 永久存储 | 动态 gate 持续决定更新/保留 |
...
| 心核组件 | 最近的先例 | 心核的扩展 |
|----------|-----------|-----------|
| 对称 cross-attention state | RMT (2022), RxT (2025) | 读写对称（Q/K/V 角色互换）+ layer_hook 零侵入 + 每层专用投影 |
| 专用投影解耦 LoRA | 标准 LoRA | LoRA 只管语言适配，state 路由由专用全参投影负责，扩展不受低秩限制 |
| Sleep replay → Memory MLP 固化 | GDWM, DualNet | 专用 MLP + 完整 sleep 周期（state 弱化 100%→0%），无直接先例 |
| .pt = 灵魂 | 无 | per-user 权重分化，checkpoint 即身份 |
| Memory MLP（专用长期记忆） | 无 | 独立 MLP 模块存长期记忆，不依赖 LoRA 挂载 backbone |

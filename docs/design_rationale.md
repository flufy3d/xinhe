...
**决策**：所有 32 个状态 token 完全无差别，没有角色预设。

**为什么**：
- Recurrent Memory Transformer 预设了读 token 和写 token，这是人类对"记忆应该怎么工作"的先验。但如果我们的核心假设是"让结构自己涌现"，就不应该预设这种分工。
- 类比人脑：没有哪个神经元出生时就被标记为"读神经元"。功能分化是训练（发育）的结果，不是初始条件。
- 32 个 token 足够多，模型有空间自己发展出读、写、缓存等不同角色。
